import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from decimal import Decimal as D
import pandas as pd
import pytest
from backtesting import Backtest
from fastapi.testclient import TestClient
import cutie_backtesting_provider as provider
import funding_history


@pytest.mark.parametrize("outcome", ["sl", "rule_exit"])
def test_short_signal_http_accounts_for_costs_funding_and_price_surge(
    monkeypatch, tmp_path, outcome
):
    values = [100, 99, 100, 101, 90, 88, 86, 110] + [110] * 42
    df = pd.DataFrame(
        dict(
            Open=values,
            High=[v + 1 for v in values],
            Low=[v - 1 for v in values],
            Close=values,
            Volume=[1] * len(values),
        ),
        index=pd.to_datetime([i * 300 for i in range(len(values))], unit="s", utc=True),
    )
    df.iloc[6, df.columns.get_loc("High")] = 91  # fill short limit after its creation bar

    def fetch(exchange, market, symbol, timeframe, start, end):
        assert (exchange, market, symbol, timeframe) == ("binance", "futures", "BTCUSDT", "5m")
        return df.loc[
            (df.index >= pd.to_datetime(start, unit="s", utc=True))
            & (df.index < pd.to_datetime(end, unit="s", utc=True))
        ].copy()

    funding_calls = []

    def funding(params):
        funding_calls.append(params)
        return [
            {
                "symbol": "BTCUSDT",
                "fundingTime": 2100000,
                "fundingRate": "0.001",
                "markPrice": "100",
            }
        ]

    monkeypatch.setattr(provider, "_fetch_ohlcv", fetch)
    monkeypatch.setattr(funding_history, "_request", funding)
    monkeypatch.setattr(provider, "REPORTS_DIR", tmp_path)
    monkeypatch.setattr(Backtest, "plot", lambda *args, **kwargs: None)
    request = {
        "backtest": {
            "run_id": "donchian_short",
            "provider_tool_id": "local.backtesting_py.breakout",
            "provider_params": {
                "lookback": 2,
                "exit_lookback": 1,
                "direction": "short",
                "exchange": "binance",
            },
            "symbol": "BTCUSDT",
            "market": "futures",
            "timeframe": "5m",
            "start_at": 1200,
            "end_at": 15000,
            "initial_capital": "10000",
            "fee_bps": "10",
            "slippage_bps": "5",
            "risk_policy": {
                "schema": "cutie.strategy_risk_policy.v1",
                "direction": "short",
                "leverage": 3,
            },
            "signal_execution": {
                "execution_policy": {
                    "schema": "cutie.strategy_signal_execution.v1",
                    "reference_price": "signal_bar_close",
                    "entry_mode": "limit_only",
                    "observation_timeframe": "5m",
                    "evaluation_lag_seconds": 30,
                    "indicator_history_bars": 4,
                    "sl_tp_rule": {
                        "stop_loss": {"type": "fixed_pct", "pct": "5" if outcome == "sl" else "50"},
                        "take_profit": {"type": "fixed_pct", "pct": "50"},
                    },
                },
                "cycle_limits": {
                    "daily_limit": 10,
                    "author_daily_limit": 10,
                    "cooldown_seconds": 0,
                    "day_offset_seconds": 0,
                },
            },
        }
    }
    body = TestClient(provider.app).post("/cutie/backtest", json=request).json()
    assert body["result_status"] == "success", body
    report = body["raw_report"]["strategy_signal_result"]
    assert funding_calls
    assert report["funding_evidence"]["rows"][0]["rate"] == "0.001"
    assert report["intents"][0]["signal"]["direction"] == "short"
    settlements = report["replay"]["settlements"]
    assert len(settlements) == 1
    exit_price = D("94.5") if outcome == "sl" else D(110)
    # Positive funding is a credit to shorts: -funding_paid adds 0.1 here.
    expected = (D(90) - exit_price - (D(90) + exit_price) * D("0.0015") + D("0.1")) / D(30)
    assert D(settlements[0]["net_return"]) == expected
    assert settlements[0]["closed_at"] == (2399 if outcome == "sl" else 2430)
    assert report["replay"]["risk_state"]["entries_paused"] == (outcome == "rule_exit")
    assert report["replay"]["open_signal"] is None
