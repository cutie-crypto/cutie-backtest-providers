...
def test_managed_stop_settlements_reset_extreme_and_breakeven_for_next_trade():
    from strategy_dynamic_stop import StopRules

    first, second = signal(0, "a"), signal(900, "b")
    for item in (first, second):
        item.update(stop_loss=D(90), target_prices=[D(200)])
    bars = [
        candle(0, 99, 101),
        candle(300, 95, 112),
        candle(600, 105, 109),
    ...
    result = run(
        [
            {"id": "a", "kind": "entry", "at": 0, "signal": first},
            {"id": "b", "kind": "entry", "at": 900, "signal": second},
        ],
        bars,
        stop_rules=StopRules(D(3), True),
        management_candles=bars,
        fee_bps=D(10),
        slippage_bps=D(5),
    )
    assert [(s["signal_id"], s["closed_at"]) for s in result["settlements"]] == [("a", 899), ("b", 1799)]
    assert [D(s["net_return"]) for s in result["settlements"]] == [
        (D("108.64") - 100 - D("208.64") * D(".0015")) / 100,
        (D(97) - 100 - D(197) * D(".0015")) / 100,
    ]
    assert [(u["signal_id"], u["stop_loss"], u["breakeven_active"]) for u in result["stop_updates"]] == [
        ("a", D("108.64"), True),
        ("b", D(97), False),
    ]
    assert result["stop_state"] is None and result["open_signal"] is None
    assert result["risk_state"]["consecutive_losses"] == 1


def test_managed_stop_rule_exit_clears_state_before_following_entry():
    from strategy_dynamic_stop import StopRules

    first, second = signal(0, "a"), signal(600, "b")
    for item in (first, second):
        item.update(stop_loss=D(90), target_prices=[D(200)])
    bars = [candle(0, 99, 101), candle(300, 95, 112), candle(600, 99, 101)]
    result = run(
        [
            {"id": "a", "kind": "entry", "at": 0, "signal": first},
            {"id": "exit", "kind": "rule_exit", "at": 600, "price": "112"},
            {"id": "b", "kind": "entry", "at": 600, "signal": second},
        ],
        bars,
        stop_rules=StopRules(D(3), True),
        management_candles=bars,
    )
    assert result["settlements"][0]["closed_at"] == 600
    assert result["open_signal"]["id"] == "b"
    assert result["open_signal"]["stop_loss"] == 90
    assert result["stop_state"]["breakeven_active"] is False
    assert result["stop_state"]["extreme"] == 100
